...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.nickName = '';
      $.isLogin = true;
      UA = `jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${
        Math.random * 98 + 1
  ...
  // if (!res) {
  //   $.http
  //     .get({ url: 'https://purge.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/cfd.json' })
  //     .then((resp) => {})
...
async function cfd() {
  try {
    ...
    $.result.push(`【京东账号${$.index}】${$.nickName || $.UserName}`, `【🥇金币】${endInfo.ddwCoinBalance}`, `【💵财富值】${endInfo.ddwRichBalance}\n`);
  } catch (e) {
    $.logErr(e);
  }
}
        ...
        if (err) {
          ...
          if (data.iRet === 0 || data.sErrMsg === 'success') {
            if (data.Data.StagePrizeInfo) {
              console.log(
                `领取助力奖励成功：获得${data.Data.ddwCoin}金币 ${data.Data.StagePrizeInfo.ddwMoney}财富 ${
                  data.Data.StagePrizeInfo.strPrizeName && !data.Data.StagePrizeInfo.ddwMoney ? data.Data.StagePrizeInfo.strPrizeName : `0元`
                }红包`
              );
            } else {
  ...
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        ...
        )}&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}&strPgUUNum=${token['farm_jstoken']}&strVersion=1.0.1&dwIsReJoin=1`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
...
function taskUrl(function_path, body = '', dwEnv = 7) {
  ...
  url += `&_stk=${getStk(url)}`;
  url += `&_ste=1&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(
...
function decrypt(time, stk, type, url) {
  ...
  if (stk) {
    ...
    return encodeURIComponent(
      [''.concat(timestamp.toString()), ''.concat($.fingerprint.toString()), ''.concat($.appId.toString()), ''.concat($.token), ''.concat(hash2), ''.concat('3.0'), ''.concat(time)].join(';')
    );
  } else {
