...
const $ = jd_env.nv("京喜财富岛");
const jdCookieNode = $.isNode() ? require("./jdCookie.js") : "";
// const notify = $.isNode() ? require('./sendNotify') : '';
$.CryptoJS = $.isNode() ? require("crypto-js") : CryptoJS;
let UA = `jdapp;iPhone;10.0.5;${Math.ceil(Math.random() * 2 + 12)}.${Math.ceil(
  Math.random() * 4
)};${randomString(40)};`;
...
`);
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      UA = `jdapp;iPhone;10.0.5;${Math.ceil(
        Math.random() * 2 + 12
    ...
    for (let j = 0; j < $.InviteLists.length && $.canHelp; j++) {
      ...
      if (res && res.iRet == 0) {
        console.log(
          `助力成功: 获得${
            (res.Data &&
              res.Data.GuestPrizeInfo &&
              res.Data.GuestPrizeInfo.strPrizeName) ||
            ""
          }`
        );
        ...
        if (
          res.sErrMsg.indexOf("助力次数达到上限") > -1 ||
          res.iRet === 2232 ||
          res.sErrMsg.indexOf("助力失败") > -1
        ) {
          break;
        }
      } else {
...
async function run() {
  try {
    ...
    if ($.HomeInfo) {
      ...
      console.log(
        `等级:${$.HomeInfo.dwLandLvl} 当前金币:${$.HomeInfo.ddwCoinBalance} 当前财富:${$.HomeInfo.ddwRichBalance} 助力码:${$.HomeInfo.strMyShareId}`
      );
    }
...
async function StoryInfo() {
  try {
    if ($.HomeInfo.StoryInfo && $.HomeInfo.StoryInfo.StoryList) {
      ...
      if (
        $.HomeInfo.StoryInfo.StoryList[0].dwType == 4 &&
        ((res && res.iRet == 0) || res == "")
      ) {
        await pickshell(4);
        await $.wait(1000);
        ...
        $.HomeInfo.StoryInfo.StoryList[0].dwType == 1 &&
        ((res && res.iRet == 0) || res == "")
      ) {
        if (
          res &&
        ...
        $.HomeInfo.StoryInfo.StoryList[0].dwType == 2 &&
        ((res && res.iRet == 0) || res == "")
      ) {
    ...
    if ($.buildList) {
      for (let i in $.buildList) {
        ...
        if (CollectCoin && CollectCoin.ddwCoinBalance) {
          console.log(
            `收集金币:${CollectCoin.ddwCoin} 当前剩余:${CollectCoin.ddwCoinBalance}`
          );
  ...
  try {
    ...
    if ($.Aggrtask && $.Aggrtask.Data && $.Aggrtask.Data.Sign) {
      if ($.Aggrtask.Data.Sign.dwTodayStatus == 0) {
        ...
        if (flag) {
          let additional = `&ptag=&ddwCoin=${ddwCoin}&ddwMoney=${ddwMoney}&dwPrizeType=${dwPrizeType}&strPrizePool${
            (strPrizePool && "=" + strPrizePool) || ""
          }&dwPrizeLv=${dwPrizeLv}`;
    ...
    if (
      $.Aggrtask &&
      $.Aggrtask.Data &&
      $.Aggrtask.Data.Employee &&
      $.Aggrtask.Data.Employee.EmployeeList
    ) {
      if (
        $.Aggrtask.Data &&
        $.Aggrtask.Data.Employee &&
        ...
        console.log(
          `\n领取邀请奖励(${
            $.Aggrtask.Data.Employee.EmployeeList.length || 0
          }/${$.Aggrtask.Data.Employee.dwNeedTotalPeople || 0})`
        );
    ...
    try {
      ...
      for (i = 1; num--; i++) {
        ...
        let res = await taskGet(
          ...
          `&ptag=&strBuildIndex=${strBuildIndex}`
        );
        if (res && res.iRet == 0) {
          console.log(
            `当前气球次数:${res.dwTodaySpeedPeople} 金币速度:${res.ddwSpeedCoin}`
          );
...
async function RubbishOper() {
  try {
    // 倒垃圾
    await $.wait(2000);
    $.QueryRubbishInfo = await taskGet(
    ...
    if (
      ...
      $.QueryRubbishInfo.Data.StoryInfo.StoryList
    ) {
      for (let i of $.QueryRubbishInfo.Data.StoryInfo.StoryList) {
        let res = "";
        if (i.strStoryId == 3) {
          console.log(`\n倒垃圾`);
          $.RubbishOper = await taskGet(
            `story/RubbishOper`,
            "_cfd_t,bizCode,dwEnv,dwRewardType,dwType,ptag,source,strZone",
            "&ptag=&dwType=1&dwRewardType=0"
          );
          if (
            $.RubbishOper &&
            $.RubbishOper.Data &&
            $.RubbishOper.Data.ThrowRubbish &&
            $.RubbishOper.Data.ThrowRubbish.Game &&
            $.RubbishOper.Data.ThrowRubbish.Game.RubbishList
          ) {
            for (let j of $.RubbishOper.Data.ThrowRubbish.Game.RubbishList) {
              console.log(`放置[${j.strName}]等待任务完成`);
              res = await taskGet(
                `story/RubbishOper`,
                "_cfd_t,bizCode,dwEnv,dwRewardType,dwRubbishId,dwType,ptag,source,strZone",
                `&ptag=&dwType=2&dwRewardType=0&dwRubbishId=${j.dwId}`
              );
              await $.wait(2000);
            }
            if (
              res &&
              res.Data &&
              res.Data.RubbishGame &&
              res.Data.RubbishGame.AllRubbish &&
              res.Data.RubbishGame.AllRubbish.dwIsGameOver &&
              res.Data.RubbishGame.AllRubbish.dwIsGameOver == 1
            ) {
              console.log(
                `任务完成获得:${
                  (res.Data.RubbishGame.AllRubbish.ddwCoin &&
                    res.Data.RubbishGame.AllRubbish.ddwCoin + "金币") ||
                  ""
                }`
              );
            } else {
...
async function Guide() {
  try {
    ...
    if ($.Employtask && $.Employtask.TourGuideList) {
      ...
      for (let i of arr) {
        console.log(
          `${i.strGuideName} 收益:${i.ddwProductCoin} 支付成本:${
            i.ddwCostCoin
          } 剩余工作时长:${timeFn(Number(i.ddwRemainTm || 0) * 1000)}`
        );
        let dwIsFree = 0;
        ...
        if (num > 0 && i.ddwRemainTm == 0) {
          res = await taskGet(
            ...
            `&ptag=&strBuildIndex=${i.strBuildIndex}&dwIsFree=${dwIsFree}&ddwConsumeCoin=${ddwConsumeCoin}`
          );
          if (res.iRet == 0) {
            console.log(`雇佣成功`);
            num--;
      ...
      for (i = 1; b--; i++) {
        ...
        for (m = 1; n--; m++) {
          ...
          await taskGet(
            `user/RealTmReport`,
            "",
            `&dwIdentityType=0&strBussKey=composegame&strMyShareId=${$.ComposeGameState.strMyShareId}&ddwCount=5`
          );
        }
        console.log("合成珍珠");
        let res = await taskGet(
...
async function ActTask() {
  try {
    ...
    if ($.Biztask && $.Biztask.Data && $.Biztask.Data.dwStatus != 4) {
      console.log(`\n牛牛任务`);
      if (
        $.Biztask.Data.dwStatus == 3 &&
        $.Biztask.Data.dwTotalTaskNum &&
        $.Biztask.Data.dwCompleteTaskNum &&
        $.Biztask.Data.dwTotalTaskNum == $.Biztask.Data.dwCompleteTaskNum
      ) {
        res = await taskGet(
          `story/ActTaskAward`,
          "_cfd_t,bizCode,dwEnv,ptag,source,strZone",
      ...
      for (let i in $.Biztask.Data.TaskList) {
        ...
        if (item.dwAwardStatus != 2 && item.dwCompleteNum === item.dwTargetNum)
          continue;
        console.log(
          `任务 ${item.strTaskName} ${item.dwCompleteNum}/${item.dwTargetNum}`
        );
        if (
          item.dwAwardStatus == 2 &&
          item.dwCompleteNum === item.dwTargetNum
        ) {
          res = await taskGet(
            `Award1`,
            "_cfd_t,bizCode,dwEnv,ptag,source,strZone,taskId",
            `&ptag=&taskId=${item.ddwTaskId}`
          );
          if (res.ret == 0) {
            ...
            if (res.data.prizeInfo.ddwCoin || res.data.prizeInfo.ddwMoney) {
              console.log(
                `${item.strTaskName} 领取奖励:${
                  (res.data.prizeInfo.ddwCoin &&
                    res.data.prizeInfo.ddwCoin + "金币") ||
                  ""
                } ${
                  (res.data.prizeInfo.ddwMoney &&
                    res.data.prizeInfo.ddwMoney + "财富") ||
                  ""
                }`
              );
            } else {
        ...
        if (
          ...
          if (item.dwOrderId == 2) {
            ...
            res = await taskGet(
              ...
              `&ptag=&taskId=${item.ddwTaskId}&configExtra=`
            );
          }
          await $.wait(1000);
          res = await taskGet(
    ...
    if ($.task && $.task.data && $.task.data.userTaskStatusList) {
      ...
      for (let i in $.task.data.userTaskStatusList) {
        ...
        if (item.awardStatus != 2 && item.completedTimes === item.targetTimes)
          continue;
        console.log(
          `任务 ${item.taskName} (${item.completedTimes}/${item.targetTimes})`
        );
        if (item.awardStatus == 2 && item.completedTimes === item.targetTimes) {
          res = await taskGet(
            `Award`,
            "_cfd_t,bizCode,dwEnv,ptag,source,strZone,taskId",
            `&ptag=&taskId=${item.taskId}`
          );
          if (res.ret == 0) {
            if (res.data.prizeInfo) {
              res.data.prizeInfo = $.toObj(res.data.prizeInfo);
        ...
        if (item.dateType == 2) {
          ...
          if (
            item.completedTimes < item.targetTimes &&
            [1, 2, 3, 4].includes(item.orderId)
          ) {
            if (
              item.taskName.indexOf("捡贝壳") > -1 ||
              item.taskName.indexOf("赚京币任务") > -1
            )
              continue;
            ...
            for (i = 1; b--; i++) {
              ...
              res = await taskGet(
                "DoTask",
                "_cfd_t,bizCode,configExtra,dwEnv,ptag,source,strZone,taskId",
                `&ptag=&taskId=${item.taskId}&configExtra=`
              );
              await $.wait(5000);
            }
            res = await taskGet(
...
function printRes(res, msg = "") {
  if (
    res.iRet == 0 &&
    (res.Data || res.ddwCoin || res.ddwMoney || res.strPrizeName)
  ) {
    let result = res;
    if (res.Data) {
      result = res.Data;
    ...
    if (
      ...
      result.ddwMoney ||
      result.strPrizeName ||
      (result.StagePrizeInfo && result.StagePrizeInfo.strPrizeName)
    ) {
      console.log(
        `${msg}获得:${(result.ddwCoin && " " + result.ddwCoin + "金币") || ""}${
          (result.ddwMoney && " " + result.ddwMoney + "财富") || ""
        }${(result.strPrizeName && " " + result.strPrizeName + "红包") || ""}${
          (result.StagePrizeInfo &&
            result.StagePrizeInfo.strPrizeName &&
            " " + result.StagePrizeInfo.strPrizeName) ||
          ""
        }`
      );
      ...
      console.log(
        `${msg}获得: ${
          (result.Prize.strPrizeName &&
            "优惠券 " + result.Prize.strPrizeName) ||
          ""
        }`
      );
    } else if (res && res.sErrMsg) {
...
function getGetRequest(type, stk = "", additional = "") {
  ...
  if (type == "user/ComposeGameState") {
    ...
    if (type == "story/GetTakeAggrPages" || type == "story/RewardSigns")
      if (type == "story/GetTakeAggrPages")
        //dwEnv = 6
        type = "story/GetTakeAggrPage";
