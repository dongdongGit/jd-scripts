  ...
  try {
    ...
    if ($.Aggrtask && $.Aggrtask.Data && $.Aggrtask.Data.Sign) {
      if ($.Aggrtask.Data.Sign.dwTodayStatus == 0) {
        ...
        if (flag) {
          let e = getJxAppToken();
          let additional = `&ptag=&ddwCoin=${ddwCoin}&ddwMoney=${ddwMoney}&dwPrizeType=${dwPrizeType}&strPrizePool${(strPrizePool && '=' + strPrizePool) || ''}&dwPrizeLv=${dwPrizeLv}&strPgtimestamp=${
            e.strPgtimestamp
          }&strPhoneID=${e.strPhoneID}&strPgUUNum=${e.strPgUUNum}`;
      ...
      for (let i in $.task.data.userTaskStatusList) {
        ...
        if (item.awardStatus == 2 && item.completedTimes === item.targetTimes) {
          ...
          if (res.ret == 0) {
            ...
            if (res.data.prizeInfo.ddwCoin || res.data.prizeInfo.ddwMoney) {
              console.log(
                `${item.taskName} 领取奖励:${(res.data.prizeInfo.ddwCoin && res.data.prizeInfo.ddwCoin + '金币') || ''} ${(res.data.prizeInfo.ddwMoney && res.data.prizeInfo.ddwMoney + '财富') || ''}`
              );
            } else {
...
function printRes(res, msg = '') {
  if (res.iRet == 0 && (res.Data || res.ddwCoin || res.ddwMoney || res.strPrizeName)) {
    ...
    if (result.ddwCoin || result.ddwMoney || result.strPrizeName || (result.StagePrizeInfo && result.StagePrizeInfo.strPrizeName)) {
      console.log(
        `${msg}获得:${(result.ddwCoin && ' ' + result.ddwCoin + '金币') || ''}${(result.ddwMoney && ' ' + result.ddwMoney + '财富') || ''}${
          (result.strPrizeName && ' ' + result.strPrizeName + '红包') || ''
        }${(result.StagePrizeInfo && result.StagePrizeInfo.strPrizeName && ' ' + result.StagePrizeInfo.strPrizeName) || ''}`
      );
