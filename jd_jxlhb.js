...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let code of $.packetIdArr) {
      ...
      console.log(
        `【${$.UserName}】去助力【${code["userName"]}】邀请码：${code["strUserPin"]}`
      );
...
function getUserInfo() {
  return new Promise((resolve) => {
    ...
    const options = taskurl(
      "GetUserInfo",
      body,
      "activeId,channel,joinDate,phoneid,publishFlag,timestamp"
    );
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
...
function taskurl(function_path, body = "", stk) {
  let url = `${BASE_URL}/${function_path}?activeId=${
    $.activeId
  }&publishFlag=1&channel=7&${body}&sceneval=2&g_login_type=1&timestamp=${Date.now()}&_=${
    Date.now() + 2
  }&_ste=1`;
  const deviceId = `${
