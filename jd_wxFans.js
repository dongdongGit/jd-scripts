  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      if ($.taskList && !$.fd3) {
        ...
        if ($.energy >= 100 && !$.fd3)
          await dotask("startDraw", "&drawType=03");
      } else {
        console.log("你已经开满啦");
      }
    }
...
function getActCk() {
  return new Promise((resolve) => {
    $.get(
      taskUrl(
        "/wxFansInterActionActivity/activity/590580",
        `activityId=${$.actid}&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined`
      ),
      (err, resp, data) => {
        try {
          if (err) {
...
//获取我的pin
function getshopid() {
  let config = taskPostUrl(
    "/customer/getSimpleActInfoVo",
    `activityId=${$.actid}`
  );
  return new Promise((resolve) => {
    $.post(config, async (err, resp, data) => {
...
function getMyPin() {
  let config = taskPostUrl(
    "/customer/getMyPing",
    `userId=${$.shopid}&token=${encodeURIComponent($.token2)}&fromType=APP`
  );
  //   console.log(config)
  return new Promise((resolve) => {
    $.post(config, async (err, resp, data) => {
...
function adlog() {
  let config = taskPostUrl(
    ...
    `venderId=${$.shopid}&code=69&pin=${encodeURIComponent(
      $.pin
    )}&activityId=90121061401&pageUrl=https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/590580?activityId=aae88e19901340a28b604f5cf5b61dd6&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined&subType=app&adSource=`
  );
  //   console.log(config)
  return new Promise((resolve) => {
    $.post(config, async (err, resp, data) => {
...
function getActinfo(end) {
  return new Promise((resolve) => {
    ...
    $.post(
      taskPostUrl("/wxFansInterActionActivity/activityContent", body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
            ...
            if (data.result) {
              ...
              if (actinfo) {
                ...
                console.log(
                  `福袋1 ${actorinfo.prizeOneStatus} 福袋2 ${actorinfo.prizeTwoStatus} 福袋3 ${actorinfo.prizeThreeStatus}`
                );
                console.log(
                  actorinfo.nicknameShow +
                    "：" +
                    (actorinfo.energyValue + actorinfo.fansLoveValue) +
                    "福气值"
                );
...
function dotask(id, body) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl(
        `/wxFansInterActionActivity/${id}`,
        `activityId=${$.actid}&uuid=${$.uuid}${body ? body : ""}`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (info && info.result) {
              ...
              if (id == "startDraw") {
                if (info.data.drawOk) {
                  console.log(`    抽奖成功,活动 ${info.data.drawInfo.name}`);
                } else {
...
function taskUrl(url, body) {
  ...
  return {
    ...
    headers: {
      ...
      Referer:
        "https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/590580?activityId=aae88e19901340a28b604f5cf5b61dd6&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined",
      "user-agent":
