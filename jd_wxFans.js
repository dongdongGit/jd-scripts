...
async function main() {
  ...
  if (
    $.actorInfo.prizeOneStatus &&
    $.actorInfo.prizeTwoStatus &&
    $.actorInfo.prizeThreeStatus
  ) {
    console.log(`已完成抽奖`);
    return;
  }
...
async function luckDraw() {
  ...
  let nowUseValue =
    Number($.activityData.actorInfo.fansLoveValue) +
    Number($.activityData.actorInfo.energyValue);
  if (
    nowUseValue >= $.activityData.actConfig.prizeScoreOne &&
    $.activityData.actorInfo.prizeOneStatus === false
  ) {
    console.log(`开始第一次抽奖`);
    $.drawType = "01";
  ...
  if (
    nowUseValue >= $.activityData.actConfig.prizeScoreTwo &&
    $.activityData.actorInfo.prizeTwoStatus === false
  ) {
    console.log(`开始第二次抽奖`);
    $.drawType = "02";
    await takePostRequest("startDraw");
  ...
  if (
    nowUseValue >= $.activityData.actConfig.prizeScoreThree &&
    $.activityData.actorInfo.prizeThreeStatus === false
  ) {
    console.log(`开始第三次抽奖`);
    $.drawType = "03";
    await takePostRequest("startDraw");
...
async function doTask() {
  $.runFalag = true;
  if (
    $.activityData.task1Sign &&
    $.activityData.task1Sign.finishedCount === 0
  ) {
    console.log(`执行每日签到`);
  ...
  if ($.activityData.task2BrowGoods && $.runFalag) {
    if (
      $.activityData.task2BrowGoods.finishedCount !==
      $.activityData.task2BrowGoods.upLimit
    ) {
      needFinishNumber =
        Number($.activityData.task2BrowGoods.upLimit) -
        Number($.activityData.task2BrowGoods.finishedCount);
      console.log(`开始做浏览商品任务`);
      $.upFlag = true;
      for (
        let i = 0;
        i < $.activityData.task2BrowGoods.taskGoodList.length &&
        needFinishNumber > 0 &&
        $.runFalag;
        i++
      ) {
  ...
  if ($.activityData.task3AddCart && $.runFalag) {
    if (
      $.activityData.task3AddCart.finishedCount !==
      $.activityData.task3AddCart.upLimit
    ) {
      needFinishNumber =
        Number($.activityData.task3AddCart.upLimit) -
        Number($.activityData.task3AddCart.finishedCount);
      console.log(`开始做加购商品任务`);
      $.upFlag = true;
      for (
        let i = 0;
        i < $.activityData.task3AddCart.taskGoodList.length &&
        needFinishNumber > 0 &&
        $.runFalag;
        i++
      ) {
  ...
  if ($.activityData.task4Share && $.runFalag) {
    if (
      $.activityData.task4Share.finishedCount !==
      $.activityData.task4Share.upLimit
    ) {
      needFinishNumber =
        Number($.activityData.task4Share.upLimit) -
        Number($.activityData.task4Share.finishedCount);
      console.log(`开始做分享任务`);
      $.upFlag = true;
  ...
  if ($.activityData.task5Remind && $.runFalag) {
    if (
      $.activityData.task5Remind.finishedCount !==
      $.activityData.task5Remind.upLimit
    ) {
  ...
  if ($.activityData.task6GetCoupon && $.runFalag) {
    if (
      $.activityData.task6GetCoupon.finishedCount !==
      $.activityData.task6GetCoupon.upLimit
    ) {
      needFinishNumber =
        Number($.activityData.task6GetCoupon.upLimit) -
        Number($.activityData.task6GetCoupon.finishedCount);
      console.log(`开始做领取优惠券`);
      $.upFlag = true;
      for (
        let i = 0;
        i < $.activityData.task6GetCoupon.taskCouponInfoList.length &&
        needFinishNumber > 0 &&
        $.runFalag;
        i++
      ) {
  ...
  if ($.activityData.task7MeetPlaceVo && $.runFalag) {
    if (
      $.activityData.task7MeetPlaceVo.finishedCount !==
      $.activityData.task7MeetPlaceVo.upLimit
    ) {
      console.log(`执行逛会场`);
      $.upFlag = true;
...
async function takePostRequest(type) {
  ...
  switch (type) {
    ...
    case "getMyPing":
      ...
      body = `userId=${$.shopid}&token=${encodeURIComponent(
        $.token
      )}&fromType=APP`;
      break;
    case "activityContent":
      url =
        "https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activityContent";
      body = `activityId=${$.activityID}&pin=${encodeURIComponent($.pin)}`;
      break;
    case "getActMemberInfo":
      url = "https://lzkjdz-isv.isvjcloud.com/wxCommonInfo/getActMemberInfo";
      body = `venderId=${$.shopid}&activityId=${
        $.activityID
  ...
  switch (type) {
    ...
    case "activityContent":
      if (data.data && data.result && data.count === 0) {
        ...
        $.nowUseValue =
          Number($.actorInfo.fansLoveValue) + Number($.actorInfo.energyValue);
      } else {
        console.log(JSON.stringify(data));
      }
      break;
...
function getPostRequest(url, body) {
  let headers = {
    ...
    Referer:
      ...
      "?activityId=" +
      $.activityID +
      "&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined",
    "user-agent": $.UA,
...
function accessLogWithAD() {
  let config = {
    ...
    body: `venderId=${$.shopid}&code=69&pin=${encodeURIComponent(
      $.pin
    )}&activityId=${
      $.activityID
    }&pageUrl=https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/${
      $.activityID
    }?activityId=${
      $.activityID
    }&shareuserid4minipg=&shopid=undefined&subType=app&adSource=`,
  };
