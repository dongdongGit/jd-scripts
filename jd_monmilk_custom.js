...
================Loon==============
[Script]
cron "10 0,7,23 * * *" script-path=https://github.com/dongdongGit/jd-scripts/blob/main/jd_monmilk_custom.js, tag=赚京豆
 */
const $ = new Env("蒙牛定制活动");
...
async function doTaskList() {
  for (task of $.taskConfig) {
    if (task.taskType == 2) {
      console.log('task',task);
      doTask(task.taskItem.itemId, task.taskType);
    }
  }
}
...
async function getConfig() {
  return new Promise((resolve) => {
    let request_params = {
      url: `https://jdjoy.jd.com/module/task/draw/get?configCode=be1ca16bbc094babb0ed2f9b14363ed2&unionCardCode=`,
      headers: {
        Cookie: cookie,
      },
    };
    $.get(request_params, async (error, response, data) => {
      try {
        ...
        $.activityBaseInfo = result?.data?.activityBaseInfo

        if ($.activityBaseInfo && $.activityBaseInfo.endTime < new Date().getTime()) {
          console.log('活动已结束');
          return;
        }

        $.taskConfig = result?.data?.taskConfig;
      } catch (e) {
        $.logErr(e, response);
      } finally {
        resolve();
...
async function doTask(itemId, taskType) {
  return new Promise((resolve) => {
    let request_params = {
      ...
      data: {
        configCode: $.activityBaseInfo.configCode,
        itemId: itemId,
        taskType: taskType,
      },
    };
    console.log(request_params);
    $.post(request_params, async (error, response, data) => {
      try {
        const result = JSON.parse(data);
