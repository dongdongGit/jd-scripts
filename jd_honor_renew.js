    ...
    setTimeout(() => {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          scorePerLottery =
            data.data.result.userInfo.scorePerLottery ||
            data.data.result.userInfo.lotteryMinusScore;
          if (
            data.data.result.raiseInfo &&
            data.data.result.raiseInfo.levelList
          )
            scorePerLottery =
              data.data.result.raiseInfo.levelList[
                data.data.result.raiseInfo.scoreLevel
              ];
          ...
          for (let i = 0; i < data.data.result.taskVos.length; i++) {
            console.log(
              "\n" +
                ...
                "-" +
                (data.data.result.taskVos[i].status === 1
                  ? `已完成${data.data.result.taskVos[i].times}-未完成${data.data.result.taskVos[i].maxTimes}`
                  : "全部已完成")
            );
            //签到
            if (data.data.result.taskVos[i].taskName === "邀请好友助力") {
              console.log(
                `您的好友助力码为:${data.data.result.taskVos[i].assistTaskDetailVo.taskToken}`
              );
              $.newShareCodes.push(
              ...
              for (let code of $.newShareCodes) {
                ...
                await harmony_collectScore(
                  code,
                  data.data.result.taskVos[i].taskId
                );
                await $.wait(2000);
              }
              ...
              if ([27].includes(data.data.result.taskVos[i].taskType)) {
                await healthyDay_getLotteryResult(
                  data.data.result.taskVos[i].taskId
                );
              } else {
              ...
              [0, 12, 13].includes(data.data.result.taskVos[i].taskType)
            ) {
              if (data.data.result.taskVos[i].status === 1) {
                await harmony_collectScore(
                  data.data.result.taskVos[i].simpleRecordInfoVo.taskToken,
                  data.data.result.taskVos[i].taskId
                );
              }
              ...
              for (
                let j = 0;
                j < (data.data.result.userInfo.lotteryNum || 0);
                j++
              ) {
                if (appId === "1E1NYwqc") {
            ...
            let list =
              data.data.result.taskVos[i].productInfoVos ||
              data.data.result.taskVos[i].followShopVo ||
              data.data.result.taskVos[i].shoppingActivityVos ||
              data.data.result.taskVos[i].browseShopVo;
            for (
              let k = data.data.result.taskVos[i].times;
              k < data.data.result.taskVos[i].maxTimes;
              k++
            ) {
                ...
                if (list[j].status === 1) {
                  ...
                  if (list[j].itemId) {
                    await harmony_collectScore(
                      ...
                      list[j].itemId,
                      1,
                      data.data.result.taskVos[i].waitDuration
                    );
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      let url = {
        ...
        body: `functionId=${lotteryResultFunPrefix}_getLotteryResult&body={"appId":"${appId}"${
        ...
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            if (data.data.result.raiseInfo)
              scorePerLottery = parseInt(
                data.data.result.raiseInfo.nextLevelScore
              );
            if (
              parseInt(data.data.result.userScore) >= scorePerLottery &&
              scorePerLottery
            ) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      let url = {
        ...
        body: `functionId=${homeDataFunPrefix}_getLotteryResult&body={"appId":"${appId}"${
          taskId ? ',"taskId":"' + taskId + '"' : ""
        }}&client=wh5&clientVersion=1.0.0`,
      };
      $.post(url, async (err, resp, data) => {
