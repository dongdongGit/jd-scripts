...
================Loon==============
[Script]
cron "30 2,9 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_sign_xd.js,tag=京喜签到-喜豆

===============Surge=================
京喜签到-喜豆 = type=cron,cronexp="30 2,9 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_sign_xd.js

============小火箭=========
京喜签到-喜豆 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jx_sign_xd.js, cronexpr="30 2,9 * * *", timeout=3600, enable=true
 */
!(function (t, r) {
...
function signhb() {
  return new Promise((resolve) => {
    $.get(taskUrl('signhb/query', 'type=0&signhb_source=5&smp=&ispp=1&tk='), async (err, resp, data) => {
      try {
        if (err) {
          ...
          const { commontask, sqactive } = data;
          $.sqactive = sqactive;
...
function dotask(task) {
  return new Promise((resolve) => {
    $.get(taskUrl('signhb/dotask', `task=${task}&signhb_source=5&ispp=1&sqactive=${$.sqactive}&tk=`), async (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err));
          ...
          data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1]);
          if (data.ret === 0) {
            console.log(`完成任务 获得${data.sendxd}喜豆`);
