...
async function all(cookie, jrBody) {
  ...
  switch (stop) {
    ...
    default:
      ...
      // await JDUserSignPre(Wait(stop), 'JDCustomized', '京东商城-定制', '3bhgbFe5HZcFCjEZf2jzp3umx4ZR'); //京东定制
...
function notify() {
  return new Promise((resolve) => {
    try {
      ...
      const disa =
        !notify || amount ? `【温馨提示】:  检测到${$nobyda.disable ? `上次执行意外崩溃, ` : ``}已禁用${notify ? `${amount}个` : `所有`}接口, 如需开启请前往BoxJs或查看脚本内第114行注释.\n` : ``;
          ...
          if (error) {
            ...
            if (data.match(/prize/)) {
              ...
              if (cc.data.prizeBean) {
                ...
                merge.JDShake.notify += `${also ? `\n` : ``}京东商城-摇摇: ${also ? `多次, ` : ``}获得满${cc.data.prizeCoupon.quota}减${cc.data.prizeCoupon.discount}优惠券→ ${
                  cc.data.prizeCoupon.limitStr
                }`;
              } else {
          ...
          if (data.match(/enActK/)) {
            ...
            if (!params) {
              ...
              let signInfo = (od.floorList || [])
                .filter((o) => o.template == 'signIn' && o.signInfos && o.signInfos.params && o.signInfos.params.match(/enActK/))
                .map((o) => o.signInfos)
                .pop();
              if (signInfo) {
    ...
    setTimeout(() => {
      const JDMCUrl = {
        url: `https://api.m.jd.com/client.action?functionId=getNewsInteractionLotteryInfo&appid=smfe${
          id ? `&body=${encodeURIComponent(`{${id.sign ? `"sign":${id.sign},` : ``}"interactionId":${id.id}}`)}` : ``
        }`,
        headers: {
          Cookie: KEY,
        },
...
function nobyda() {
  ...
  const write = (value, key) => {
    ...
    if (isNode) {
      try {
        if (!node.fs.existsSync(node.path.resolve(__dirname, NodeSet))) node.fs.writeFileSync(node.path.resolve(__dirname, NodeSet), JSON.stringify({}));
