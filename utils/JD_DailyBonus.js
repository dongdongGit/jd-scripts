...
async function all(cookie, jrBody) {
  ...
  switch (stop) {
    ...
    //京东金融 京豆双签
      ...
      await JingDongGetCash(Wait(stop));
      ...
      await JingRongDoll(Wait(stop), 'JDDouble', '金融京豆-双签', 'F68B2C3E71', '', '', '', 'jingdou');
      break;
  }
  await Promise.all([
    //TotalSteel(), //总钢镚查询
...
function notify() {
  return new Promise((resolve) => {
    try {
      ...
      const disa =
        !notify || amount ? `【温馨提示】:  检测到${$nobyda.disable ? `上次执行意外崩溃, ` : ``}已禁用${notify ? `${amount}个` : `所有`}接口, 如需开启请前往BoxJs或查看脚本内第118行注释.\n` : ``;
