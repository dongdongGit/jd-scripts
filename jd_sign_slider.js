...
async function by() {
  ...
  return new Promise(async (t) => {
    $.post(o, (z, A, B) => {
      try {
        const { ret: F, msg: G, data: { result: H } = {} } = JSON.parse(B);
        $.token = H.tk;
...
function bB(b, f) {
  ...
  return (
    /(y+)/i.test(k) && (k = k.replace(RegExp.$1, ''.concat(j.getFullYear()).substr(4 - RegExp.$1.length))),
...
class bK {
  ...
  ['recognize']() {
    const { ctx: e, w: f, bg: h } = this,
      { width: j, height: k } = this.patch,
  ...
  ['runWithCanvas']() {
    const { createCanvas: e, Image: f } = require('canvas'),
      j = e();
    ...
    const { naturalWidth: p, naturalHeight: q } = l;
    j.width = p;
    j.height = q;
    k.clearRect(0, 0, p, q);
    k.drawImage(l, 0, 0, p, q);
    const s = p,
      { naturalWidth: t, naturalHeight: u } = m,
...
async function bL(f) {
  ...
  let { data: k } = await b7.post('https://cactus.jd.com/request_algo?g_ty=ajax', _0x8a8aa8, j);
  let l = k,
    m = l.data.result,
...
async function bR(e, f, g) {
  ...
  t = v
    .filter((H) => u[H])
    .map((H) => H + ':' + (H == 'body' ? b6.SHA256(s[H]).toString() : s[H]))
    .join('&');
  let w = {};
...
class bT {
  ...
  ['moveToAndCollect']({ x: a, y: b, duration: e }) {
    let g = 0;
    let h = 0;
    let i = 0;
