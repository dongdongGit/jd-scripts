...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      try {
        await jdMh(
          "https://anmp.jd.com/babelDiy/Zeus/3bMo2AgbRYYfZt83qHLZ3ruVtrtG/index.html"
        );
      } catch (e) {
        $.logErr(e);
      }
      ...
      try {
        if (err) {
          ...
          if (data.data && data.data.drawflag) {
            if (
              $.prize.filter((vo) => vo.prizeLevel === data.data.level).length >
              0
            ) {
              console.log(
                `获得${
                  $.prize.filter((vo) => vo.prizeLevel === data.data.level)[0]
                    .prizename
                }`
              );
              $.beans += $.prize.filter(
                (vo) => vo.prizeLevel === data.data.level
              )[0].beansPerNum;
            } else {
              console.log(`抽奖 未中奖`);
            }
...
function taskUrl(function_id, body = "") {
  body = `activeid=${$.info.activeId}&token=${
    $.info.actToken
  }&sceneval=2&shareid=&_=${new Date().getTime()}&callback=query&${body}`;
  return {
