...
async function jdWish() {
  ...
  for (let i = 0; i < $.taskList.length; ++i) {
    ...
    if (task["taskId"] === 1 && task["status"] !== 2) {
      ...
      console.log(`去做任务：${task.taskName}`);
      if (task["itemId"]) await doTask({ itemId: task["itemId"], taskId: task["taskId"], mpVersion: "3.4.0" });
      else await doTask({ taskId: task["taskId"], mpVersion: "3.4.0" });
      await $.wait(3000);
    }
  }
  await getTaskList();
...
function taskTuanUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(JSON.stringify(body))}&appid=swat_miniprogram&osVersion=5.0.0&clientVersion=3.1.3&fromType=wxapp&timestamp=${
