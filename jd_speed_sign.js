...
async function taskList() {
  return new Promise((resolve) => {
    $.get(
      taskUrl("ClientHandleService.execute", {
        version: "3.1.0",
...
async function queryJoy() {
  return new Promise((resolve) => {
    $.get(taskUrl("ClientHandleService.execute", { method: "queryJoyPage", data: { channel: 1 } }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
            ...
            if (jd_helpers.safeGet(data)) {
              ...
              if (data.code === 0 && data.data) {
                if (data.data.taskInfo.isTaskLimit === 0) {
                  ...
                  if (activeType !== 3) videoBrowsing = activeType === 1 ? 5 : 10;
...
async function cash() {
  return new Promise((resolve) => {
    $.get(taskUrl("MyAssetsService.execute", { method: "userCashRecord", data: { channel: 1, pageNum: 1, pageSize: 20 } }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
...
function wheelsHome() {
  return new Promise((resolve) => {
    $.get(taskGetUrl("wheelsHome", { linkId: "toxw9c5sy9xllGBr3QFdYg" }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
...
function wheelsLottery() {
  return new Promise((resolve) => {
    $.get(taskGetUrl("wheelsLottery", { linkId: "toxw9c5sy9xllGBr3QFdYg" }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
...
function apTaskList() {
  return new Promise((resolve) => {
    $.get(taskGetUrl("apTaskList", { linkId: "toxw9c5sy9xllGBr3QFdYg" }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
...
function apDoTask(taskType, taskId, channel, itemId) {
  ...
  return new Promise((resolve) => {
    $.get(taskGetUrl("apDoTask", { linkId: "toxw9c5sy9xllGBr3QFdYg", taskType: taskType, taskId: taskId, channel: channel, itemId: itemId }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(`${$.name} API请求失败，请检查网路重试`);
...
var __Oxb24bc = [
  "\x6C\x69\x74\x65\x2D\x61\x6E\x64\x72\x6F\x69\x64\x26",
  "\x73\x74\x72\x69\x6E\x67\x69\x66\x79",
  "\x26\x61\x6E\x64\x72\x6F\x69\x64\x26\x33\x2E\x31\x2E\x30\x26",
  "\x26",
  ...
  "",
  "\x61\x70\x69\x3F\x66\x75\x6E\x63\x74\x69\x6F\x6E\x49\x64\x3D",
...
function invite2() {
  ...
  let dataString = `functionId=TaskInviteService&body={"method":"participateInviteTask","data":{"channel":"1","encryptionInviterPin":"${encodeURIComponent(
    inviterId
  )}","type":1}}&appid=market-task-h5&uuid=&_t=${t}`;
...
function invite() {
  let t = +new Date();
  let inviterId = ["", ""][Math.floor(Math.random() * 2)];
  var headers = {
    Host: "api.m.jd.com",
    accept: "application/json, text/plain, */*",
    "content-type": "application/x-www-form-urlencoded",
