...
$.shareCodes = [];
$.blackInfo = {};
const JX_FIRST_RUNTASK = $.isNode()
  ? process.env.JX_FIRST_RUNTASK && process.env.JX_FIRST_RUNTASK === 'xd'
    ? '5'
    : '1000'
  : $.getdata('JX_FIRST_RUNTASK') && $.getdata('JX_FIRST_RUNTASK') === 'xd'
  ? '5'
  : '1000';
if ($.isNode()) {
    ...
    $.get(taskUrl('signhb/query', body), async (err, resp, data) => {
      try {
        if (err) {
          ...
          const { smp, commontask, sharetask, signlist = [] } = data;
    ...
    $.get(JDtaskUrl('SignedInfo', `_=${Date.now()}`), async (err, resp, data) => {
      try {
        if (err) {
          ...
          const {
            data: { jd_sign_status, jx_sign_status, double_sign_status },
          } = data;
          if (data.retCode === 0) {
