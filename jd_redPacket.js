  ...
  return new Promise((resolve) => {
    const options = {
      ...
      headers: {
        ...
        Referer:
          "https://h5.m.jd.com/babelDiy/Zeus/yj8mbcm6roENn7qhNdhiekyeqtd/index.html",
            ...
            if (data.code === "0" && data["busiCode"] === "0") {
              if (data.result.luckyDrawData.actId) {
                if (data.result.luckyDrawData.redPacketId) {
                  console.log(
                    `券后9.9抽奖获得【红包】：${data.result.luckyDrawData.quota}元`
                  );
                } else {
                  console.log(
                    `券后9.9抽奖获得【优惠券】：${data.result.luckyDrawData.discount}元：${data.result.luckyDrawData.prizeName}，${data.result.luckyDrawData.quotaDesc}`
                  );
                }
              } else {
  ...
  if ($.taskHomePageData && $.taskHomePageData.code === 0) {
    ...
    if ($.taskInfo && $.taskInfo.length > 0) {
      ...
      for (let item of $.taskInfo) {
        console.log(
          `${item.title.slice(-6)}   ${
            item.alreadyReceivedCount ? item.alreadyReceivedCount : 0
          }/${item.requireCount}      ${item.innerStatus === 4 ? "是" : "否"}`
        );
      }
      for (let item of $.taskInfo) {
...
async function red() {
  ...
  if (
    ...
    if ($.h5activityIndex["data"]["result"]["assistants"]) {
      $.assistants =
        $.h5activityIndex["data"]["result"]["assistants"].length || 0;
    }
  }
  if (
    $.h5activityIndex &&
    ...
    $.h5activityIndex &&
    $.h5activityIndex.data &&
    $.h5activityIndex.data["biz_code"] === 20001
  ) {
    //20001:红包活动正在进行，可拆
    const redPacketId =
      $.h5activityIndex["data"]["result"]["redpacketInfo"]["id"];
    if (redPacketId) $.redPacketId.push(redPacketId);
    console.log(
      `\n\n当前待拆红包ID:${
        $.h5activityIndex["data"]["result"]["redpacketInfo"]["id"]
      }，进度：再邀${
        $.h5activityIndex["data"]["result"]["requireAssistNum"]
      }个好友，开第${$.hasSendNumber + 1}个红包。当前已拆红包：${
        $.hasSendNumber
      }个，剩余${
        $.h5activityIndex["data"]["result"]["remainRedpacketNumber"]
      }个红包待开，已有${$.assistants}好友助力\n\n`
    );
    const waitOpenTimes =
      $.h5activityIndex["data"]["result"]["redpacketInfo"]["waitOpenTimes"] ||
      0;
    ...
    $.h5activityIndex &&
    $.h5activityIndex.data &&
    $.h5activityIndex.data["biz_code"] === 20002
  ) {
...
function taskHomePage() {
  return new Promise((resolve) => {
    $.post(
      taskUrl(arguments.callee.name.toString(), { clientInfo: {} }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`\n${$.name}: API查询请求失败 ‼️‼️`);
...
function startTask(taskType) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl(arguments.callee.name.toString(), data),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`\n${$.name}: API查询请求失败 ‼️‼️`);
...
function receiveTaskRedpacket(taskType) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskUrl(arguments.callee.name.toString(), body),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`\n${$.name}: API查询请求失败 ‼️‼️`);
      ...
      try {
        if (err) {
          ...
          if (
            ...
            const rewards =
              $.h5activityIndex["data"]["result"]["rewards"] || [];
    ...
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            // data = JSON.parse(data);
            if (type === "1" && functionId === "reportCcTask")
...
function taskUrl(functionId, body) {
  return {
    url: `${JD_API_HOST}?appid=jd_mp_h5&functionId=${functionId}&loginType=2&client=jd_mp_h5&t=${
