...
!(async () => {
  ...
  await updateShareCodesCDN(
    "https://cdn.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_shareCodes.json"
  );
    ...
    if (total > need) {
      ...
      for (let i = 0; i < times; i++) {
        ...
        if (lotteryRes.code === 200) {
          ...
          if (
            lotteryRes.data["prizeId"] &&
            lotteryRes.data["type"] !== "99" &&
            lotteryRes.data["type"] !== "3" &&
            lotteryRes.data["type"] !== "8" &&
            lotteryRes.data["type"] !== "9"
          ) {
      ...
      if (message)
        allMessage += `京东账号${$.index} ${
          $.nickName
        }\n${message}抽奖详情查看 https://blindbox5g.jd.com/#/myPrize${
...
function taskurl(body = {}) {
  return {
    url: `${JD_API_HOST}?appid=blind-box&functionId=blindbox_prod&body=${JSON.stringify(
      body
    )}&t=${Date.now()}&loginType=2`,
    headers: {
