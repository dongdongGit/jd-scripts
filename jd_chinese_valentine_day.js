...
async function run() {
  try {
    ...
    if ($.userInfo) {
      ...
      console.log(
        `当前有${$.userInfo.coins}枝玫瑰🌹 等级${$.userInfo.level} ${
          $.userInfo.id
        } ${
          ($.userInfo.is_join_team == 1 && "已组成特工小队") || "未组特工小队"
        }`
      );
    ...
    if ($.taskList == "") {
      ...
      console.log(
        `每日打卡地图(${$.taskList.light_maps_num}/${$.taskList.maps_num})`
      );
      if ($.taskList.light_maps_num < $.taskList.maps_num) {
        ...
        if ($.mapList != "" && $.mapList.data) {
          for (let i = 0; i < $.mapList.data.length; i++) {
            ...
            if ($.oneTask.is_light != 0 || $.oneTask.is_join_team != 1)
              continue;
      ...
      console.log(
        `浏览并关注店铺(${$.taskList.task_shops_num}/${$.taskList.shops_num})`
      );
      if ($.taskList.task_shops_num < $.taskList.shops_num) {
        ...
        if ($.shopList != "" && $.shopList.data) {
          for (let i = 0; i < $.shopList.data.length; i++) {
            ...
            console.log(
              `关注店铺 ${$.oneTask.name} ${$.oneTask.is_follow} ${$.oneTask.is_task}`
            );
            $.resTask = "";
      ...
      console.log(
        `浏览并加购商品(${$.taskList.task_products_num}/${$.taskList.products_num})`
      );
      ...
      console.log(
        `浏览会场(${$.taskList.view_meeting_num}/${$.taskList.meeting_num})`
      );
      ...
      console.log(
        `店铺会员开卡(${$.taskList.open_card_num}/${$.taskList.card_num})`
      );
    ...
    do {
      ...
      if ($.homeInfo && $.homeInfo.is_coins_enough) {
        ...
        if ($.resTask && $.resTask.letter_info) {
          ...
          console.log(
            `${$.resTask.letter_info.peroration}\n${
        ...
        console.log(
          `------------------------------\n${
            ($.homeInfo && $.homeInfo.coins) ||
            ($.userInfo && $.userInfo.coins) ||
            0
          }枝玫瑰🌹 ${
            ($.homeInfo &&
              "需要" + $.homeInfo.need_coins + "枝玫瑰🌹才能兑换[情书]") ||
            "不满足兑换[情书]"
          }`
        );
      }
        ...
        try {
          ...
          if (typeof res == "object") {
            if (res.status_code) {
              ...
              if (
                res.message.indexOf("助力已达") > -1 &&
                res.message.indexOf("上限") > -1
              ) {
                $.inviteCeil = 1;
              ...
              if ($.resTask && $.resTask.add_coins >= 0) {
                if (type.indexOf("map_team_invite?inviter_id=") == 0)
                  $.joinTeamId = "";
                let msg = "";
                if ($.resTask.beans) {
