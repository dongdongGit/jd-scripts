...
function shareCodesFormat() {
  return new Promise(async (resolve) => {
    ...
    newShareCodes = newShareCodes.filter((item) => {
      return fullist.indexOf(item) == -1;
    });
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    if ($.isNode()) {
      ...
      if ($.getdata('jd_plantbean_inviter'))
        $.shareCodesArr = $.getdata('jd_plantbean_inviter')
          .split('\n')
          .filter((item) => !!item);
...
function getUA() {
  ...
  ep = encodeURIComponent(
    JSON.stringify({
      ...
      ts: Date.now(),
      ridx: -1,
      cipher: { sv: 'EG==', ad: adod, od: od, ov: 'Ctq=', ud: adod },
      ciphertype: 5,
      version: '1.2.0',
