...
const Opencardtw = $.isNode()
  ? process.env.Opencardtw
    ? process.env.Opencardtw
    : false
  : false;
            ...
            if (task.assignmentType == 3) {
              ...
              if (Opencardtw) {
                ...
                await doTask(
                  ...
                  $.enpid,
                  task.encryptAssignmentId,
                  task.ext.brandMemberList[0].itemId,
                  7
                );
              } else {
...
function getCode(source, actid) {
  return new Promise(async (resolve) => {
    const options = taskPostUrl(
      "superBrandTaskList",
      `{"source":"${source}","activityId":${actid},"assistInfoFlag":1}`
    );
    //   console.log(options)
    $.post(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (
            ...
            if (data.data.result && data.data.result.taskList) {
              $.taskList = data.data.result.taskList.filter(
                (x) =>
                  x.assignmentType == 3 ||
                  x.assignmentType == 7 ||
                  x.assignmentType == 0
              ...
              let result = data.data.result.taskList.filter(
                (x) => x.assignmentType == 2
              )[0];
...
function taskPostUrl(functionid, body) {
  const time = Date.now();
  return {
    url: `https://api.m.jd.com/api?functionId=${functionid}&appid=ProductZ4Brand&client=wh5&t=${time}&body=${encodeURIComponent(
      body
    )}`,
    body: "",
    headers: {
