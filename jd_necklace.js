          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.rtn_code === 0) {
                if (data.data.biz_code === 0) {
                  console.log(
                    `点点券领取成功,获得${data.data.result.giftScoreNum},当前共有${data.data.result.totalScoreNum}\n`
                  );
          ...
          if (safeGet(data)) {
            ...
            if (data.rtn_code === 0) {
              if (data.data.biz_code === 0) {
                $.taskConfigVos = data.data.result.taskConfigVos;
                $.exchangeGiftConfigs =
                  data.data.result.exchangeGiftConfigs || [];
                ...
                const config = $.exchangeGiftConfigs.filter(
                  (item) => item["giftType"] === 1
                );
                if (config && config[0]) {
...
function taskPostUrl(function_id, body = {}) {
  const time =
    ...
    8 * 60 * 60 * 1000;
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=coupon-necklace&loginType=2&client=coupon-necklace&t=${time}&body=${escape(
      JSON.stringify(body)
    )}&uuid=${uuid}`,
