...
#
##京东国际盲盒
#5 7,12,23 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_global_mh.js |ts >> /scripts/logs/jd_global_mh.log 2>&1
...
## 宠汪汪
01 0-8/2,9,10-23/2 * * *  node /scripts/jd_joy.js |ts >> /scripts/logs/jd_joy.log 2>&1
## 宠汪汪积分兑换京豆
...
#
## 必须要的默认定时任务请勿删除
#47 0 * * * docker_entrypoint.sh |ts >> /scripts/logs/default_task.log 2>&1
